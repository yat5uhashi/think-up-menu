...
- **email の正規化**：**小文字に正規化して保存**し、一意性判定とログインは**大文字小文字を区別しない**。
  - `Alice@Example.COM` と `alice@example.com` は**同一アドレス**として扱う（重複登録は 400、どちらの表記でもログイン可）。
  - RFC 5321 上ローカル部は大文字小文字を区別しうるが、同節で「その区別に依存するのは非推奨」とされており、実在のメールプロバイダも区別しない。区別すると同一受信箱に対して重複アカウントが生まれ、メール確認・パスワードリセット（フェーズ2）が破綻するため同一視する。
  - Django 標準の `BaseUserManager.normalize_email` はドメイン部しか小文字化しないため、`UserManager` で全体を小文字化する。
...
- [ ] 大文字混じりで登録すると小文字に正規化して保存される（`Alice@Example.COM` → `alice@example.com`）。
- [ ] 大文字違いの同一アドレスでの重複登録は 400 になる。
- [ ] 小文字で登録したアドレスに、大文字混じりの入力でもログインできる。
